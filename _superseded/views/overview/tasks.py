...
class TasksView(BaseView):
    ...
    def dispatch_request(self, **kwargs):
        ...
        if self.task_id and self.task_result_id:
            self.template = 'scrapydash/task_job_results.html'
            self.query_task_job_results()
        elif self.task_id:
            self.template = 'scrapydash/task_results.html'
            self.query_task_results()
        else:
            self.template = 'scrapydash/tasks.html'
            self.query_tasks()
    ...
    def query_tasks(self):
        ...
        # Removed Flask-SQLAlchemy reference - now using FastAPI with pure SQLAlchemy
        # http://sqlalchemy.org/library.html#sqlalchemy.engine.result.ResultProxy
        ...
        with db.session.no_autoflush:
            for index, task_result in enumerate(task_results.items,
                ...
                if with_job:  # To show task_job_result in task_results.html
                    self.template = 'scrapydash/task_results_with_job.html'
